...
if voice_score is not None and hand_score is not None:
    ...
    # =============================
    report = f"""
NeuroSense AI Clinical Report

Date: {datetime.now().strftime("%Y-%m-%d %H:%M")}

Voice Score: {voice_score:.2f}%
Handwriting Score: {hand_score:.2f}%
...
"""
    ...
    def create_pdf():
        file = "NeuroSense_Report.pdf"
        styles = getSampleStyleSheet()
        doc = SimpleDocTemplate(file)
        content = []

        content.append(Paragraph("NeuroSense AI Clinical Report", styles['Title']))
